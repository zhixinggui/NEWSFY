...
- (void)layoutButtons{
    ...
    [titile setObject:arr forKey:@"arr"];
    [titile writeToFile:filename atomically:YES];
...
- (void)updatePageView:(NSNotification *)notifition{
    ...
    for (NSString *key in self.tmpPageViewDic) {
       
//        UIView *pageView =[[UIView alloc]init];
//        pageView = [self.tmpPageViewDic objectForKey:key];
        [tmpArray addObject: [self.tmpPageViewDic objectForKey:key]];
        i++;
    }
...
- (void)updateTitles:(NSNotification *)notifition{
    ...
    //输入写入
    NSMutableDictionary *titile = [[NSMutableDictionary alloc]initWithContentsOfFile:filename];
    NSMutableArray *arrT = [[NSMutableArray alloc]initWithArray:[titile valueForKey:@"arr"]];
    for (NSString *str in  arrT) {
        if ([str isEqualToString:notifition.object]) {
            [arrT removeObject:str];
            break;
        }
    }
    [titile setObject:arrT forKey:@"arr"];
    [titile writeToFile:filename atomically:YES];
