...
@implementation ForUWebViewController{UIButton *share;
...
- (void)viewDidLoad {
    ...
    self.shareView.backgroundColor=    self.shareView.backgroundColor=[UIColor clearColor];;
...
- (void)setupHeader:(UIScrollView *)table
{
    ...
    refreshHeader.beginRefreshingOperation = ^{
        ...
        if ( ( newscId-self.newsminid)>0) {
          ...
          [NSTimer scheduledTimerWithTimeInterval:0.6 target:self selector:@selector(Time1) userInfo:nil repeats:NO];
            [self setFavButton];
...
- (void)requestPost:(int)newsId andTagView:(int)tagView{
    ...
    NSDictionary *result = [self.newsArray objectAtIndex:newsId];
...
-(void)favClick{
    ...
    if (favFlag) {
        ...
        if ([dicUserInfo valueForKey:@"userid"]) {
            ...
            NSString *newsid = [[self.newsArray objectAtIndex:newscId] valueForKey:@"newsid"];
            [parameters setValue:newsid forKey:@"newsId"];
            [parameters setValue:[[self.newsArray objectAtIndex:newscId] valueForKey:@"newsstyleid"] forKey:@"newsStyleId"];
            ...
            [request requestData:url url:parameters dic:^(NSMutableDictionary*dic1){
                ...
                if ([s isEqualToString:@"0"]) {
                    ...
                    //取消收藏
                    NSDictionary *result = [self.newsArray objectAtIndex:newscId];
                    ...
                    [parameters setValue:[dicUserInfo valueForKey:@"userid"] forKey:@"userId"];
                    [parameters setValue:[result valueForKey:@"newsid"] forKey:@"newsId"];
                    [parameters setValue:[result valueForKey:@"newsstyleid"] forKey:@"newsStyleId"];
...
-(void)inserFoot{
    ...
    [parameters setValue:newsid forKey:@"newsId"];
    [parameters setValue:[[self.newsArray objectAtIndex:newscId] valueForKey:@"newsstyleid"] forKey:@"newsStyleId"];
    [parameters setValue:[[self.newsArray objectAtIndex:newscId] valueForKey:@"newstitle"] forKey:@"newsTitle"];
    [parameters setValue:[[self.newsArray objectAtIndex:newscId] valueForKey:@"newsfirstimg"] forKey:@"newsFirstImg"];
    
    [parameters setValue:[[self.newsArray objectAtIndex:newscId] valueForKey:@"newstime"] forKey:@"newsTime"];
    ...
    NSString *culm =[NSString stringWithFormat:@"Tag%@",[[self.newsArray objectAtIndex:newscId] valueForKey:@"newsstyleid"]];
...
-(void)share:(UIButton *)sender{
    ...
    NSString *newsstyleid = [[self.newsArray objectAtIndex:newscId] valueForKey:@"newsstyleid"] ;
    NSString *table = [dicT valueForKey: newsstyleid];
    
    
    
    NSString *url = [NSString stringWithFormat:@"http://115.29.176.50/interface/index.php/Home/ForU/shar?table=%@&newsId=%@",table,newsid];

    if(sender.tag==1){
        //分享到新浪
       
        [shareWay shareToSina:[[self.newsArray objectAtIndex:newscId] valueForKey:@"newstitle"] andImg:[UIImage imageNamed:@"watingImag"] andURL:url andView:self];
        
    }
    if(sender.tag==2){
        //分享到qq
      [shareWay shareToQQ:[[self.newsArray objectAtIndex:newscId] valueForKey:@"newstitle"] andImg:[UIImage imageNamed:@"watingImag"] andURL:url andView:self];
    ...
    if(sender.tag==3){
        ...
        [shareWay shareToQZone:[[self.newsArray objectAtIndex:newscId] valueForKey:@"newstitle"] andImg:[UIImage imageNamed:@"watingImag"] andURL:url andView:self];
...
-(void)click:(UIButton *)sender{
    if(sender.tag==7){
        ...
        [UIView animateWithDuration:0.5 animations:^{
            ...
            NSMutableDictionary *parameters =[[NSMutableDictionary alloc]init];
            NSLog(@"%@",self.editView.editText.text);
            [parameters setValue:[dicUserInfo valueForKey:@"userid"] forKey:@"userId"];
    ...
    if (sender.tag == 9) {
        ...
        [UIView animateWithDuration:0.1 animations:^{
            ...
            cvc.newsid =[[self.newsArray objectAtIndex:newscId] valueForKey:@"newsid"];
            cvc.newsstyleid = [[self.newsArray objectAtIndex:newscId] valueForKey:@"newsstyleid"] ;
